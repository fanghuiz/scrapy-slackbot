...
class SlackBot(object):
    def __init__(self, stats, slack_bot_token, channel):
        self.stats = stats
        self.bot = slack.WebClient(token=slack_bot_token)
    ...
    def from_crawler(cls, crawler):
        ...
        if not crawler.settings.getbool(
                'SLACK_ENABLED') or not slack_bot_token or not channel:
            raise NotConfigured
    ...
    def spider_opened(self, spider):
        ...
        self.bot.chat_postMessage(channel=self.channel,
    ...
    def spider_closed(self, spider, reason):
        attachments = [{
            ...
            'fields': [{
                ...
                'Item Scraped Count',
                'value':
                self.stats.get_value('item_scraped_count') if
                self.stats.get_value('item_scraped_count') is not None else 0,
                'short':
                False
            }, {
                'title':
                'Filtered',
                'value':
                self.stats.get_value('dupefilter/filtered') if
                self.stats.get_value('dupefilter/filtered') is not None else 0,
                'short':
                False
            }],
            'footer':
